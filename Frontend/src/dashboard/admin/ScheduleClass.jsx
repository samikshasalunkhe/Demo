...
// Note: Student uses localStorage (keeps your original student UI behavior).
// If you want to use backend for students, import addStudentSchedule/getStudentSchedules here.
...
export default function ScheduleClass({ isEditable = true }) {
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto text-white">
      <h1 className="text-3xl font-bold mb-6 text-blue-400 flex items-center">📅 Diploma Timetable</h1>
      ...
      <div className="flex gap-3 mb-4">
        {views.map(v => (
          <button
            ...
            className={`px-4 py-2 rounded-lg shadow ${selectedView === v ? 'bg-blue-600 text-white' : 'bg-slate-600 text-gray-300'}`}
          >
            {v} View
          </button>
        ))}
      ...
      {selectedView === 'Student' && (
        <>
          <div className="flex flex-wrap gap-4 mb-6 items-center">
            <select value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} className="p-3 bg-slate-700 border border-gray-500 rounded-lg">
              {departments.map((dept) => <option key={dept}>{dept}</option>)}
            </select>
            <select value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="p-3 bg-slate-700 border border-gray-500 rounded-lg">
              {years.map((yr) => <option key={yr}>{yr}</option>)}
            </select>
            {isEditable && (
              <button onClick={handleAddScheduleClick} className="bg-green-600 hover:bg-green-700 text-white px-5 py-2 rounded-lg shadow">➕ Add Schedule</button>
            )}
          </div>

          <div className="overflow-auto rounded-2xl shadow-lg border border-gray-600 bg-slate-800">
            <table className="min-w-full text-sm text-white border-collapse">
              <thead>
                <tr className="bg-slate-700 text-blue-300">
                  ...
                  {timeSlots.map(slot => (
                    <th key={slot} className="px-4 py-3 text-left">
                      {(slot === '11:10–11:45' || slot === '1:45–1:55') ? (
                        <span className={slot === '11:10–11:45' ? 'text-orange-300' : 'text-yellow-300'}>{slot}</span>
                      ) : slot}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {days.map(day => (
                  <tr key={day} className="border-t border-slate-700">
                    <td className="px-4 py-3 font-semibold text-blue-300">{day}</td>
                    {timeSlots.map(slot => {
                  ...
                  facultyData.map((f, idx) => {
                    ...
                    return (
                      <tr key={id} className="border-t border-slate-700">
                        ...
                        <td className="px-3 py-2">{f.sem || f.semester}</td>
